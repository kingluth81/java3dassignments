...
public class Assignment4LA extends JPanel implements ActionListener {
    ...
    private Assignment4LA[] Assignment4LA = new Assignment4LA[3];
    Hashtable<String, MouseListener> m_MouseHashtable = null;

    public Assignment4LA( )	{
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i > 0)
                Assignment4LA[i] = new Assignment4LA(canvas3D[i]);
        ...
        lcl.addBranchGraph( createViewer( canvas3D[1], "F-L", CommonsLA.Orange, -3.5, 1, 3.5 ) );
        lcl.addBranchGraph( createViewer( canvas3D[2] , "B-R", CommonsLA.Cyan, 3.5, 1, -3.5 ) );

        BranchGroup scene = Assignment4LA.create_Scene();                  // create a one-cube scene
        TransformGroup scene_TG = new TransformGroup();
        scene.addChild(scene_TG);
        scene.addChild(CommonsLA.add_Lights(CommonsLA.White, 1));
   ...
   /* ViewerAvatar createViewerAvatar( String szText, Color3f objColor ) {
        ...
        Appearance app = CommonsLA.obj_Appearance(objColor);
   ...
   /* public void actionPerformed( ActionEvent event ) {
        ...
        for( int n = 0; n < keysArray.length; n++ )	{
            ...
            if( keyAtIndex == key ) {
                if (n == 1) {
                    canvas3D[1].addMouseListener((MouseListener) Assignment4LA[1]);
                    Assignment4LA.playSound(1);
                    canvas3D[2].removeMouseListener((MouseListener) Assignment4LA[2]);
                }
                else {
                    canvas3D[2].addMouseListener((MouseListener) Assignment4LA[2]);
                    Assignment4LA.playSound(2);
                    canvas3D[1].removeMouseListener((MouseListener) Assignment4LA[1]);
                }
            }
        }
    }
    ...
    public static void main( String[] args ) {
        ...
        frame.getContentPane().add(new Assignment4LA()); // create an instance of the class
